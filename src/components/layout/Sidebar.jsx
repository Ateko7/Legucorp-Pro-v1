...
// ─── Section icons (simple SVG paths) ────────────────────────────────────────
const icons = {
  dashboard: (
...
const sections = [
  ...
  },
  {
    ...
    icon: icons.finanzas,
    items: [
      { label: 'Cierres operativos', path: '/cierres' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Trazabilidad y Recall', path: '/trazabilidad' },
    ],
  },
  {
    title: 'Inteligencia',
...
export default function Sidebar({ onClose }) {
  ...
  return (
    <aside className="flex h-full flex-col bg-[#f2eadf] border-r border-[#dccfbe]">
      {/* Brand header */}
      <div className="flex items-center justify-between px-5 py-5 border-b border-[#dccfbe]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-stone-800 leading-tight">Legucorp Pro</p>
        ...
        <Link
          ...
          className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${
            isActive('/')
              ...
              : 'text-stone-700 hover:bg-[#e8ddd0]'
          }`}
        >
          {icons.dashboard}
          Dashboard
        ...
        {sections.map((section) => {
          ...
          return (
            <div key={section.key}>
              ...
              {isOpenSection && (
                <div className="mt-0.5 ml-4 pl-3 border-l border-[#d4cbbf] space-y-0.5 pb-1">
                  {section.items.map((item) => (
                    <Link
                      ...
                      className={`block rounded-lg px-3 py-2 text-sm transition ${
                        isActive(item.path)
                          ? 'bg-[#2f5d50] text-white font-semibold shadow-sm'
                          : 'text-stone-600 hover:bg-[#e8ddd0] hover:text-stone-800'
                      }`}
                    >
                      {item.label}
